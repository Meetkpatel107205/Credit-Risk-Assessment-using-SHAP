(() => {
  ...
  const apiBase =
    window.location.protocol === "file:" || isLocalStaticServer
      ? "http://127.0.0.1:8000"
      : "";
  ...
  fetch(`${apiBase}/openapi.json`, { method: "GET" })
    .then((res) => {
      if (res.ok) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      person_home_ownership: document.getElementById("person_home_ownership")
        .value,
      person_emp_length: parseFloat(
      ...
      cb_person_default_on_file: document.getElementById(
        "cb_person_default_on_file",
      ).value,
      cb_person_cred_hist_length: parseInt(
    ...
    try {
      ...
      showError(
        `Could not reach the ledger. ${err.message || "Check the service is running."}`,
      );
    } finally {
      setLoading(false);
    }
